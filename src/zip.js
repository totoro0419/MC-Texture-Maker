...
function put16(view, off, v) { view.setUint16(off, v, true); }
function put32(view, off, v) { view.setUint32(off, v >>> 0, true); }
...
async function inflateRaw(bytes) {
  if (typeof DecompressionStream === 'undefined') {
    throw new Error('This browser does not support deflate decompression. Use a current Chromium/Edge/Chrome browser.');
  }
  const stream = new Blob([bytes]).stream().pipeThrough(new DecompressionStream('deflate-raw'));
  return new Uint8Array(await new Response(stream).arrayBuffer());
}

export async function readZip(input, onProgress = () => {}) {
  ...
  if (eocd < 0) throw new Error('Not a supported ZIP: end-of-central-directory was not found.');
  ...
  for (let index = 0; index < entryCount; index++) {
    ...
    else if (method === 0) data = compressed.slice();
    else if (method === 8) data = await inflateRaw(compressed);
    else throw new Error(`Unsupported ZIP compression method ${method} for ${name}`);
    entries.push({ id: crypto.randomUUID(), path: name, data, directory: name.endsWith('/'), flags, method, dirty: false });
    p += 46 + nameLen + extraLen + commentLen;
    onProgress((index + 1) / entryCount, name);
  }
  return entries;
}
...
export function writeZip(entries) {
  ...
  for (const entry of files) {
    const name = textEncoder.encode(entry.path.replace(/^\/+/, ''));
  ...
  put16(ev, 4, 0);
  put16(ev, 6, 0);
  put16(ev, 8, files.length);
  put16(ev, 10, files.length);
